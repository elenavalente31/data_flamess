...
class Journal(IdentifiableEntity):
    def __init__(self, identifiers, title, languages, seal:bool, licence, apc:bool, publisher=None):
        ...
        self.apc = apc # il risultato deve essere un BOOLEAN! Vedere come fare visto che sono sì e no.
        self.Categories = []  
        self.Areas= []    

# se quando creo l'oggetto journal so già quali sonno le sue categorie e le aree allora conviene passarle alla funzione init
...
class CategoryQueryHandler:
    ...
    def getById(self, id):
        ...
        # Cerca nella tabella Category (sia ID che nome)
        cursor.execute("""
            SELECT category_id AS id, category AS name, quartile, 'Category' AS type
            FROM Category
            WHERE category = ? OR category_id = ?
        """, (id, id))
        results.extend(cursor.fetchall())

        # Cerca nella tabella Area (sia ID che nome)
        cursor.execute("""
            SELECT area_id AS id, area AS name, NULL AS quartile, 'Area' AS type
            FROM Area
            WHERE area = ? OR area_id = ?
        """, (id, id))
        results.extend(cursor.fetchall())

        # Cerca nella tabella JournalIdentifier
        cursor.execute("""
            SELECT journal_id AS id, identifier AS name, NULL AS quartile, 'Journal' AS type
        ...
        # Crea DataFrame
        import pandas as pd
        df = pd.DataFrame(results, columns=["id", "name", "quartile", "type"])
        return df

    def getAllCategories(self):
        """
        returns a data frame containing all the categories included in the database, with no repetitions.
        """
    ...
    def getAllAreas(self):
        """
        returns a data frame containing all the areas included in the database, with no repetetions.
        """
        db_path = self.db_path 
        conn = None
        try:
            ...
            print(f"Database error in getAllAreas: {e}") # Testo errore coerente
            return pd.DataFrame()
        finally:
            if conn:
                conn.close() 
    ...
    def getCategoriesWithQuartile(self, quartiles):
        """
        Restituisce un DataFrame contenente tutte le categorie con i quartili specificati
        """
        db_path = self.db_path 
        conn = None
        try:
            ...
            # Query di base che include SEMPRE category_id
    ...
    #     """
    #     Returns a DataFrame with all categories assigned to the specified area_ids. 
    #     If area_ids is empty, returns cateogries assigned to all areas
    #     """
    #     db_path = self.getDbPathOrUrl()
    #     conn = None
    #     try:
    #         conn = sqlite3.connect(db_path)
    #         if not area_ids:
    #             query = "SELECT DISTINCT C.category, C.quartile FROM Category C JOIN HasCategory HC ON C.category_id = HC.category_id"
    #             df = pd.read_sql_query(query, conn)
    #         else: 
    #             placeholders = ','.join(['?'] * len(area_ids))
    #             query = f"""SELECT DISTINCT C.category, C.quartile
    #                 FROM Category C
    #                 JOIN HasCategory HC ON C.category_id = HC.category_id
    #                 JOIN HasArea HA ON HC.journal_id = HA.journal_id
    #                 WHERE HA.area_id IN ({placeholders})"""
    #             df = pd.read_sql_query(query, conn, params=area_ids)
    #         return df
    #     except sqlite3.Error as e:
    ...
    #     db_path = self.getDbPathOrUrl()
    #     """
    #     Returns a DataFrame with all areas assigned to the specified cateogry_ids.
    #     If category_ids is empty, returns areas assigned to all categories.
    #     """
    ...
    #                 WHERE HC.category_id IN ({placeholders})"""
    #             df = pd.read_sql_query(query, conn, params=category_ids)
    #         return df
    #     except sqlite3.Error as e:
    #         print(f"Database error in getAreasAssignedCategories: {e}")
    #         return pd.DataFrame()
    #     finally:
    #         if conn:
    #             conn.close()
...
class JournalQueryHandler(QueryHandler):
    ...
    def getById(self, id):
        ...
        filter_id = f'FILTER(CONTAINS(STR(?identifier), "{id}"))' #applies the id filter to the basic query
    ...
    def getJournalWithDOAJSeal(self):
...
# Definizione class BasicQueryEngine e FullQueryEngine
